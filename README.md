...
Открой в браузере `http://127.0.0.1:8080`. Вставь текст банковской выписки в
`textarea`, выбери модель и глубину анализа, затем нажми «Обработать выписку».

Web-сервер поддерживает `deepseek-v4-flash` и `deepseek-v4-pro`. История
обработок хранится только в памяти страницы и исчезает после перезагрузки.
Для каждой записи видны расход токенов и время обработки; нажатие на запись
восстанавливает её текст, модель и глубину анализа в форме.
